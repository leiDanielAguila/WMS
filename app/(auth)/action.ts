...
export async function signup(formdata: FormData) {
  ...
  if (error) {
    ...
    redirect(
      `/signup?error=${encodeURIComponent("Sign up error " + error.message)}`
    );
    ...
    if (profileError) {
      ...
      redirect(
        `/signup?error=${encodeURIComponent(
          "Sign up error " + profileError.message
        )}`
      );
    }
  }
  ...
  return redirect(
    `/signup?message=${encodeURIComponent("Check email for confirmation")}`
  );
}
